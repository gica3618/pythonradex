...
def compute_pythonradex_model(N, Tkin, coll_partner_densities):
    source.update_parameters(N=N, Tkin=Tkin, collider_densities=coll_partner_densities)
    source.solve_radiative_transfer()
    return (
        source.Tex,
        source.upper_level_population,
        source.lower_level_population,
...
def compute_RADEX_model(N, Tkin, coll_partner_densities, transitions):
    ...
    for trans in transitions:
        freq_interval = radex_wrapper.Interval(
            min=trans.nu0 - epsilon, max=trans.nu0 + epsilon
        )
...
pythonradex_model = compute_pythonradex_model(
    N=example_N, Tkin=example_Tkin, coll_partner_densities=example_coll_dens
)
x = [
    trans.up.E / constants.k
    for trans in source.emitting_molecule.rad_transitions[: plot_max_trans_index + 1]
]
for ax, y, y_radex in zip(axes, pythonradex_model, radex_model):
    ax.plot(
        x,
        y[: plot_max_trans_index + 1],
        marker="o",
        linestyle="None",
        label="pythonradex",
    ...
    ax.plot(
        x,
        y_radex[: plot_max_trans_index + 1],
        marker="x",
        linestyle="None",
        label="RADEX",
        color=colors["RADEX"],
...
for i, Tkin in enumerate(Tkin_grid):
    for j, nH2 in enumerate(nH2_grid):
        ...
        for k, N in enumerate(N_grid):
            print(
                f"Tkin={Tkin:.2g} K, nH2 = {nH2/constants.centi**-3:.2g} cm-3,"
            ...
            try:
                ...
                if str(e) == "maximum number of iterations reached":
                    ...
                    raise
            pythonradex_model = [
                pythonradex_model[i][plot_trans_index] for i in range(4)
            ]
...
def color_pixel(ax, X, Y, j, k, color):
    ...
    ax.add_patch(
        plt.Rectangle((x0 - width / 2, y0 - height / 2), width, height, color=color)
    )
...
fig, axes = plt.subplots(
    nrows=nrows, ncols=ncols, constrained_layout=True, figsize=(6, 8)
)
...
for column, (quantity, values) in enumerate(
    zip(("Tex", "tau"), (excitation_temp, optical_depth))
):
    print(quantity)
    ...
    for i, j, k in zip(*np.isnan(ratio).nonzero()):
        print(
            f"Tkin={Tkin_grid[i]:.3} K, nH2={nH2_grid[j]/constants.centi**-3:.3g} cm-3, "
    ...
    for row, Tkin in enumerate(Tkin_grid):
        ax = axes[row, column]
        im = ax.pcolormesh(
            X, Y, ratio[row, :, :], shading="nearest", cmap=cmap, vmin=vmin, vmax=vmax
        )
        ...
        ax.set_xlim(
            np.log10(np.min(nH2_grid) / constants.centi**-3),
            np.log10(np.max(nH2_grid) / constants.centi**-3),
        )
        ax.set_ylim(
            np.log10(np.min(N_grid) / constants.centi**-2),
    ...
    if quantity == "tau":
        ...
        colorbar_label = (
            "$T_\mathrm{{ex}}^\mathrm{{pythonradex}}/T_\mathrm{{ex}}^\mathrm{{RADEX}}$"
        )
    else:
        raise RuntimeError
