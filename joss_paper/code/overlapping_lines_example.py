...
N = 2.11e14 * constants.centi**-2 / constants.kilo * Doppler
...
reference_cloud_parameters = {
    "N": N,
    "Tkin": Tkin,
    "collider_densities": {"H2": nH2},
...
# from the *.out files:
molpop_cep_tau_nu0 = {
    "treat overlap": [2.23e00, 1.70e00, 6.05e00, 1.20e01, 1.80e-01, 2.50e00],
    "ignore overlap": [3.03e00, 3.50e00, 7.98e00, 1.43e01, 2.18e-01, 2.90e00],
}
molpop_cep_Tex = {
    "treat overlap": [8.22e00, 8.55e00, 8.75e00, 8.83e00, 5.56e00, 5.80e00],
    "ignore overlap": [5.41e00, 4.86e00, 5.79e00, 7.74e00, 4.39e00, 4.64e00],
}
...
for ID, source in sources.items():
    ...
    for i, index in enumerate(line_indices):
        print(f"line {index}:")
        print(
            f"tau_nu0 = {source.tau_nu0_individual_transitions[index]:.2g}, "
            + f"Tex = {source.Tex[index]:.3g} K"
        )
        print(
            f"molpopcep: tau_nu0 = {molpop_cep_tau_nu0[ID][i]:.2g},"
            + f" Tex={molpop_cep_Tex[ID][i]:.2g}"
        )
    print("\n")

nu0s = [sources["treat overlap"].emitting_molecule.nu0[index] for index in line_indices]
nu0s = sorted(nu0s)
...
hcn = molecule.EmittingMolecule(
    datafilepath=datafilepath, line_profile_type=line_profile_type, width_v=width_v
)
molpop_cep_tau = {
    "treat overlap": np.zeros_like(nu),
    "ignore overlap": np.zeros_like(nu),
}
molpop_cep_Stot = {
    "treat overlap": np.zeros_like(nu),
    "ignore overlap": np.zeros_like(nu),
...
for ID, source in sources.items():
    spectrum = source.spectrum(
        solid_angle=solid_angle, nu=nu, output_type="flux density"
    )
    ax.plot(
        v / constants.kilo,
        spectrum * 1e26,
    ...
    ax.plot(
        v / constants.kilo,
        molpop_cep_spectrum[ID] * 1e26,
        label=f"{ID}\n(MOLPOP-CEP)",
        linestyle=linestyles[ID],
        color=colors["molpop-cep"],
    )
