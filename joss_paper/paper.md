...
![For the two spherical geometries, we computed the flux using the slab formula (as done by `RADEX`) and a formula appropriate for a sphere (as done by `pythonradex`). The figure shows the ratio of the fluxes as function of optical depth.\label{fig:flux_comparison_sphere}](code/flux_comparison_spherical_geometries.pdf)
