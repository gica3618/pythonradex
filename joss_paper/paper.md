...
![**The ratio of CO 2-1 excitation temperature (left column) and flux (right column) computed with `pythonradex` and `RADEX` for an LVG slab. Each panel shows a parameter space of column density and kinetic temperature with a fixed H$_2$ density. To avoid that differences in flux calculation affect this comparison, the `RADEX` fluxes are computed from the `RADEX` level populations using the same method as `pythonradex`. Values outside the colorbar range are shown in red.**\label{fig:pythonradex_vs_radex}](code/Tex_pythonradex_vs_radex.pdf)
