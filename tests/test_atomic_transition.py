...
def test_fast_Tex():
    ...
    assert np.all(
        atomic_transition.Tex(x1=np.array((0.2, 0.1)), x2=np.zeros(2), **kwargs)[0] == 0
    )
...
def test_K12():
    ...
    for Tkin in (120, np.array((23, 34))):
        ...
        K12 = atomic_transition.compute_K12(
            K21=K21, g_up=g_up, g_low=g_low, Delta_E=Delta_E, Tkin=Tkin
        )
...
class TestLineProfile:
    ...
    rect_line_profile = atomic_transition.RectangularLineProfile(
        nu0=nu0, width_v=width_v
    )
    ...
    def test_normalisation(self):
        for profile in self.profiles.values():
            integrated_line_profile = np.trapezoid(
                profile.phi_nu(self.test_nu), self.test_nu
            )
            integrated_line_profile_v = np.trapezoid(
                profile.phi_v(self.test_v), self.test_v
            )
    ...
    def test_profile_shape_gauss(self):
        ...
        for phi, x_axis, width in zip(
            ...
            assert np.isclose(
                2 * np.abs(x_axis[max_index] - x_axis[half_max_index]),
                width,
                rtol=4e-2,
                atol=0,
            )
    ...
    def test_phi_nu_averaging(self):
        ...
        def test_func(func, grid_width, n_grid_elements, rtol):
            fine_nu_grid = np.linspace(
                self.nu0 - grid_width * self.width_nu,
                self.nu0 + grid_width * self.width_nu,
                n_grid_elements,
            )
            for profile in self.profiles.values():
                expected_average = np.trapezoid(
                    func(fine_nu_grid) * profile.phi_nu(fine_nu_grid), fine_nu_grid
                )
                expected_average /= np.trapezoid(
                    profile.phi_nu(fine_nu_grid), fine_nu_grid
                )
        ...
        def nasty(nu):
            return np.where(
                (np.abs(nu - self.nu0) < 6 * self.width_nu)
    ...
    def test_freq_coverage(self):
        for lp in (self.rect_line_profile, self.gauss_line_profile):
            ...
            assert np.all(
                lp.covers_frequency(np.array(mixed)) == np.array(expected_mixed_answer)
            )
...
class TestLevel:
    ...
    def test_Boltzmann_level_population(self):
        ...
        assert (
            ...
            == self.g * np.exp(-self.E / (constants.k * T)) / Z
        )
        shape = (5, 5)
        T_array = np.ones(shape) * T
        Z_array = np.ones(shape) * Z
        Boltzmann_level_population_array = self.level.Boltzmann_level_population(
            Z=Z_array, T=T_array
        )
...
class TestTransition:
    ...
    test_emission_line = atomic_transition.EmissionLine(
        up=up, low=low, A21=A21, line_profile_type=line_profile_type, width_v=width_v
    )
    ...
    def test_emission_line_constructor(self):
        ...
        assert (
            self.test_emission_line.tau_kwargs["g_up"] == self.test_emission_line.up.g
        )
        assert (
            self.test_emission_line.tau_kwargs["g_low"] == self.test_emission_line.low.g
        )
