...
filepaths = [
    os.path.join(data_folder, "co.dat"),
    os.path.join(data_folder, "co_with_additional_comments.dat"),
]
lamda_data = [
    LAMDA_file.read(filepath, read_frequencies=False) for filepath in filepaths
]
lamda_data_nu0_read = [
    LAMDA_file.read(filepath, read_frequencies=True) for filepath in filepaths
]
lamda_data_nu0_qn_read = [
    LAMDA_file.read(filepath, read_frequencies=True, read_quantum_numbers=True)
...
def test_levels():
    ...
    for i in range(len(filepaths)):
        ...
        for levels in all_levels:
            ...
            assert np.isclose(
                test_level.E / (constants.h * constants.c),
                211.404098498 / constants.centi,
                atol=0,
            )
            assert test_level.index == test_level_index
...
def test_coll_transitions():
    for i in range(len(filepaths)):
        all_coll_transitions = [
            data[i]["collisional transitions"] for data in all_read_data
        ]
        for coll_transitions in all_coll_transitions:
            ...
            assert test_coll_trans_pH2.up.index == 14
            assert test_coll_trans_pH2.low.index == 9
            coeffs_pH2 = (
                np.array(
                    (
                        1.484e-11,
            ...
            test_coll_trans_oH2 = coll_transitions["ortho-H2"][
                test_coll_trans_index_oH2
            ]
            Tkin_data_oH2 = Tkin_data_pH2
            assert np.all(test_coll_trans_oH2.Tkin_data == Tkin_data_oH2)
            assert test_coll_trans_oH2.up.index == 20
            assert test_coll_trans_oH2.low.index == 10
            coeffs_oH2 = (
                np.array(
                    (
                        4.949e-13,
