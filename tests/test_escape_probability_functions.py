...
Taylor_geometries = [
    {
        "analytical": epf.beta_analytical_static_sphere,
...
medium_tau = np.array((1.0,))
large_tau = np.array((100.0,))
...
def test_Taylor_betas():
    for geo in Taylor_geometries:
        ...
        assert np.all(
            beta(analytical_taus) == epf.clip_prob(geo["analytical"](analytical_taus))
        )
...
def test_LVG_sphere_RADEX_beta():
    ...
    assert (
        epf.beta_LVG_sphere_RADEX(medium_tau) > 0
    ...
    assert np.all(
        epf.beta_LVG_sphere_RADEX(grt7) == epf.beta_LVG_sphere_RADEX_gtr7(grt7)
    )
    positive_less7 = np.array((0.01, 6.5))
    assert np.all(
        epf.beta_LVG_sphere_RADEX(positive_less7)
