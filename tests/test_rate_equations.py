...
class RateEqGenerator:
    ...
    def rate_eq_iterator(self):
        for treat_line_overlap, ext_bg in itertools.product([True, False], [0, cmb]):
            for Td, taud in zip((0, T_dust), (0, tau_dust)):
                yield self.generate_rate_eq(
                    treat_line_overlap=treat_line_overlap,
                    ext_background=ext_bg,
                    T_dust=T_dust,
                    tau_dust=tau_dust,
                )
...
class TestGeneral:
    ...
    rate_eq_generator = RateEqGenerator(
    ...
    def test_no_dust(self):
        for Td, taud in zip((0, T_dust), (0, tau_dust)):
            ...
            rate_eq = self.rate_eq_generator.generate_rate_eq(
                treat_line_overlap=True, ext_background=0, T_dust=Td, tau_dust=taud
            )
    ...
    def test_Ieff_nu0(self):
        general_kwargs = {
            "n_levels": self.test_molecule.n_levels,
        ...
        for rate_eq in self.rate_eq_generator.rate_eq_iterator():
            ...
            tau_tot_nu0 = [
                self.test_molecule.get_tau_tot_nu(line_index=i, **tau_tot_kwargs)(
                    line.nu0
                )
            ...
            S_dust_nu0 = helpers.B_nu(
                nu=self.test_molecule.nu0, T=T_dust(self.test_molecule.nu0)
            )
            ...
            Ieff = rate_eq.Ieff_nu0(
                **general_kwargs,
                Iext_nu0=Iext_nu0,
                beta_nu0=beta_nu0,
                S_dust_nu0=S_dust_nu0,
                tau_dust_nu0=tau_dust_nu0,
    ...
    def test_mixed_term_nu0(self):
        ...
        for rate_eq in self.rate_eq_generator.rate_eq_iterator():
            ...
            for t, trans in enumerate(self.test_molecule.rad_transitions):
                iup, ilow = trans.up.index, trans.low.index
                expected_mixed_term[iup, ilow] = (
                    (1 - beta_nu0[t]) / tau_tot_nu0[t] * tau_line_nu0[t] * A21[t]
                )
    ...
    def test_tau_line_functions(self):
        ...
        zero_tau_rate_eq = self.rate_eq_generator.get_zero_tau_rate_eq(
            ext_background=cmb
        )
...
class TestOverlapStuff:
    ...
    def get_mixed_term_transition_pairs(self, l, lprime):
        ...
        for downward_trans in downward_transitions_from_lprime:
            if self.test_molecule.rad_transitions[downward_trans].low.index == l:
                transition_pairs_to_consider.append(
                    [
                        downward_trans,
                    ]
                    * 2
    ...
    def mixed_term_averaged(self, tau_tot_functions, tau_line_functions):
        ...
        for l, lprime in itertools.product(
            range(self.test_molecule.n_levels), repeat=2
        ):
            if l == lprime:
            ...
            for trans_pair in transition_pairs_to_consider:
                ...
                def mixed(nu):
                    ...
                    return np.where(
                        tau_tot == 0, 0, (1 - beta) * tau_line / tau_tot * A21
                    )
                ...
                mixed_lprime_l += sign * self.test_molecule.rad_transitions[
                    trans_pair[1]
    ...
    def test_mixed_term_averaged(self):
        ...
        for rate_eq in self.rate_eq_generator.rate_eq_iterator():
            ...
            expected = self.mixed_term_averaged(
                tau_tot_functions=tau_tot_functions,
                tau_line_functions=tau_line_functions,
            )
...
class TestGammaR:
    ...
    def all_rate_eqs_iterator(self):
        return itertools.chain(
            TestGeneral.rate_eq_generator.rate_eq_iterator(),
        ...
        for rate_eq in self.all_rate_eqs_iterator():
            ...
            if not rate_eq.treat_line_overlap:
                ...
                Ieff_nu0 = rate_eq.Ieff_nu0(
                    n_levels=n_levels,
                    Iext_nu0=rate_eq.Iext_nu0,
                    beta_nu0=beta_nu0,
                    S_dust_nu0=rate_eq.S_dust_nu0,
                ...
                expected_GammaR = (
                    rate_eq.U_nu0 + rate_eq.V_nu0 * Ieff_nu0 - mixed_term_nu0
                )
            else:
                tau_line_functions = rate_eq.get_tau_line_functions(
                    level_population=level_pop
                ...
                mixed = rate_eq.mixed_term_averaged(
                    tau_tot_functions=tau_tot_functions,
                    tau_line_functions=tau_line_functions,
                )
            ...
            expected_diag = -(
                np.sum(expected_GammaR, axis=0) - expected_GammaR.diagonal()
            )
...
def test_square_line_profile_averaging():
    ...
    Ieff_nu0 = rate_eq_nu0.Ieff_nu0(
        n_levels=n_levels,
        Iext_nu0=rate_eq_nu0.Iext_nu0,
        beta_nu0=beta_nu0,
        S_dust_nu0=rate_eq_nu0.S_dust_nu0,
        trans_low_index=trans_low_index,
        trans_up_index=trans_up_index,
        tau_dust_nu0=rate_eq_nu0.tau_dust_nu0,
        tau_tot_nu0=tau_tot_nu0,
        surely_no_dust=rate_eq_nu0.surely_no_dust,
    )
    V_Ieff_nu0 = rate_eq_nu0.V_nu0 * Ieff_nu0
    tau_tot_functions = rate_eq_avg.get_tau_tot_functions(level_population=level_pop)
    ...
    tau_line_functions = rate_eq_avg.get_tau_line_functions(level_population=level_pop)
    mixed_term_averaged = rate_eq_avg.mixed_term_averaged(
        tau_tot_functions=tau_tot_functions, tau_line_functions=tau_line_functions
    )
    GammaR_nu0 = rate_eq_nu0.GammaR(level_population=level_pop)
    GammaR_averaged = rate_eq_avg.GammaR(level_population=level_pop)
    for X_nu0, X_averaged in [
        (V_Ieff_nu0, V_Ieff_averaged),
