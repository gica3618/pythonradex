...
class Test_Overlapping:
    ...
    def generate_cloud(
        self, N, line_profile_type, treat_line_overlap, coll_dens, geometry
    ):
    ...
    def test_optically_thin(self):
        ...
        level_pops = []
        for treat_line_overlap, lp, geo in itertools.product(
            (True, False), self.line_profile_types, self.geometries
        ):
            source = self.generate_cloud(
                N=self.N["thin"],
            ...
            assert not np.allclose(
                source.level_pop, Boltzmann_level_population, rtol=0, atol=1e-2
            )
    ...
    def test_LTE(self):
        ...
        for lp, (ID, Nvalue), geo in itertools.product(
            self.line_profile_types, self.N.items(), self.geometries
        ):
            source = self.generate_cloud(
                N=Nvalue,
                line_profile_type=lp,
            ...
            assert np.allclose(
                source.level_pop, Boltzmann_level_population, atol=0, rtol=1e-2
            )
    ...
    def test_spectra_thick_LTE(self):
        ...
        for treat_line_overlap, geo in itertools.product(
            (True, False), self.geometries
        ):
            source = self.generate_cloud(
                N=self.N["thick"],
