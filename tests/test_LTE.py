...
N_values = {
    "co": np.array((1e12, 1e15, 1e18)) / constants.centi**2,
...
collider_densities = {
    "co": {"ortho-H2": 1e9 / constants.centi**3},
...
def test_LTE():
    ...
    for filename, geo, lp, ng, treat_overlap in itertools.product(
        ...
        for N in N_values[specie]:
            ...
            selection = Boltzmann_level_population > min_level_pop[specie] * np.max(
                Boltzmann_level_population
            )
            assert np.allclose(
                source.level_pop[selection],
                Boltzmann_level_population[selection],
                atol=1e-6,
                rtol=1e-2,
            )
