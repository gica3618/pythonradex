...
datafilepath = {
    "CO": os.path.join(datafolder, "co.dat"),
...
def get_general_test_source(specie, width_v):
    return radiative_transfer.Source(
        datafilepath=datafilepath[specie],
...
def iterate_all_source_params():
    return itertools.product(
        geometries, line_profile_types, use_ng_options, treat_overlap_options
    )


def iterate_allowed_source_params():
    for geo, lp, use_ng, treat_overlap in iterate_all_source_params():
        if allowed_geo_param_combination(
            geometry=geo, line_profile_type=lp, treat_line_overlap=treat_overlap
        ):
...
class TestUpdateParameters:
    ...
    def test_should_be_updated(self):
        ...
        assert radiative_transfer.Source.should_be_updated(
            new_func_or_number=1.1, old=func1
        )
    ...
    def test_initial_param_setting(self):
        for source in general_source_iterator(specie="CO", width_v=1 * constants.kilo):
            ...
            self.verify_cloud_params(
                source=source, new_params=self.standard_params, original_params=None
            )
    ...
    def test_update_Tkin_colldens(self):
        for source in general_source_iterator(specie="CO", width_v=1 * constants.kilo):
            ...
            changed_coll_dens = [
                None,
                self.standard_params["collider_densities"],
                {"para-H2": 23},
                {"para-H2": 31, "ortho-H2": 34},
                {"ortho-H2": 2389},
            ]
    ...
    def test_update_ext_background(self):
        for source in general_source_iterator(specie="CO", width_v=1 * constants.kilo):
            ...
            changed_ext_bgs = [
                None,
                self.standard_params["ext_background"],
                lambda nu: nu * 2.234,
                helpers.generate_CMB_background(z=2),
            ]
    ...
    def test_update_dust(self):
        for source in general_source_iterator(specie="CO", width_v=1 * constants.kilo):
            ...
            changed_Tdust = [
                None,
                self.standard_params["T_dust"],
                some_func,
                lambda nu: nu / (100 * constants.giga) * 123,
            ]
            changed_tau_dust = [
                None,
                self.standard_params["tau_dust"],
                some_func,
    ...
    def test_update_several_parameters(self):
        for source in general_source_iterator(specie="CO", width_v=1 * constants.kilo):
            ...
            new_params = {
                "ext_background": lambda nu: cmb(nu) / 2,
            ...
            self.verify_cloud_params(
                source=source, new_params=new_params, original_params=old_params
            )
    ...
    def test_None_parameters(self):
        for source in general_source_iterator(specie="CO", width_v=1 * constants.kilo):
            old_params = {
                ...
                "collider_densities": {"para-H2": 1},
                "N": 2e12,
                "T_dust": lambda nu: np.ones_like(nu) * 145,
            ...
            source.update_parameters(**new_params)
            self.verify_cloud_params(
                source=source, new_params=new_params, original_params=current_params
            )

    def test_invalid_collider_densities(self):
        test_source = get_general_test_source(specie="CO", width_v=1 * constants.kilo)
        invalid_coll_densities = {
            "e": 1 / constants.centi**3
        }  # CO has not electron data
        with pytest.raises(ValueError):
            test_source.update_parameters(
                ext_background=cmb,
    ...
    def test_update_parameters_with_physics(self):
        ...
        for N, ext_b, Tkin, coll_dens, colliders, T_dust, tau_dust in param_iterator():
            params = {
                "N": N,
                "Tkin": Tkin,
                "ext_background": ext_b,
                "T_dust": T_dust,
                "tau_dust": tau_dust,
            }
            # put different values in case more than one collider (not really necessary...)
            coll_densities = {
                collider: coll_dens * (i + 1) for i, collider in enumerate(colliders)
            }
...
def test_ng_acceleration():
    ...
    for geo, lp, treat_overlap in itertools.product(
        geometries, line_profile_types, [True, False]
    ):
        ...
        for N, Tkin, coll_dens, ext_background in itertools.product(
            N_values, Tkin_values, coll_dens_values, ext_backgrounds
        ):
            params = {
                "N": N,
                "Tkin": Tkin,
            ...
            for ng in (True, False):
                source = radiative_transfer.Source(
                    use_Ng_acceleration=ng, **cloud_kwargs
                )
                source.update_parameters(**params)
                source.solve_radiative_transfer()
...
def test_compute_residual():
    ...
    small_tau = np.array(
        (0.7 * min_tau, min_tau / 2, min_tau / 100, min_tau / 500, min_tau / 1.01)
    )
    tau = np.array((1, 10, 2 * min_tau, min_tau, min_tau / 2))
    n_relevant_taus = (tau > min_tau).sum()
    Tex_residual = np.array((1, 2, 3, 4, 5))
    assert (
        radiative_transfer.Source.compute_residual(
            Tex_residual=Tex_residual, tau=small_tau, min_tau=min_tau
        )
        == 0
    )
    ...
    assert (
        radiative_transfer.Source.compute_residual(
            Tex_residual=Tex_residual, tau=tau, min_tau=min_tau
        )
        == expected_residual
    )
...
def test_upper_lower_level_pop():
    source = get_general_solved_test_source()
...
def test_intensity_transformation():
    ...
    T_RJ = radiative_transfer.Source.transform_specific_intensity(
        specific_intensity=specific_intensity_RJ,
        nu=nu,
        solid_angle=None,
    ...
    for output_type in ("specific intensity", "Rayleigh-Jeans", "Planck"):
        ...
        with pytest.warns(UserWarning):
            source.emission_at_line_center(
                output_type=output_type, transitions=None, solid_angle=0.45
            )
    ...
    for source in CO_HCl_model_iterator():
        for transitions in (None, [0, 2], 1):
            if transitions is None:
                ...
                nu0s = np.array(
                    [
                        source.emitting_molecule.rad_transitions[i].nu0
            ...
            for output_type in spec_output_types:
                ...
                expected_emission = source.spectrum(
                    nu=nu0s, output_type=output_type, solid_angle=solid_angle
                )
...
@pytest.mark.filterwarnings("ignore:negative optical depth")
@pytest.mark.filterwarnings("ignore:LVG sphere geometry")
def test_transition_indices_edge_cases_line_center_emission():
    invalid_indices = np.ones((2, 3, 4))
    for source in CO_HCl_model_iterator():
        all_indices = np.arange(source.emitting_molecule.n_rad_transitions)
        for output_type in (
...
def test_spectrum():
    for source in CO_HCl_model_iterator():
        ...
        specific_intensity = source.spectrum(nu=nu, output_type="specific intensity")
        for output_type in spec_output_types:
            solid_angle = 1 if output_type == "flux density" else None
            spec = source.spectrum(
                nu=nu, output_type=output_type, solid_angle=solid_angle
            )
...
@pytest.mark.filterwarnings("ignore:negative optical depth")
@pytest.mark.filterwarnings("ignore:LVG sphere geometry")
def test_spectrum_and_tau_with_0D_frequency():
    trans_index = 3
    n_channels = 30
    solid_angle = 0.25
...
class TestSpectrumPhysics:
    ...
    Tkin = 123
    collider_densities = {
        "ortho-H2": 1e5 * constants.centi**-3,
        ...
        for source in general_source_iterator(specie=self.specie, width_v=self.width_v):
            ...
            if "LVG" not in source.geometry_name:
                source.update_parameters(
                    ...
                    ext_background=cmb,
                    T_dust=self.T_dust,
                    tau_dust=self.tau_dust_thick,
                )
                source.solve_radiative_transfer()
                T_Planck = source.emission_at_line_center(output_type="Planck")
                assert np.allclose(T_Planck, self.T_dust, atol=0, rtol=1e-3)
                T_Planck_spec = source.spectrum(
                    nu=source.emitting_molecule.nu0, output_type="Planck"
                )
    ...
    def test_RJ_Planck_consistency(self):
        for source in general_source_iterator(specie=self.specie, width_v=self.width_v):
            for T_dust, tau_dust in zip((0, 45), (0, 1.2)):
                ...
                T_RJ_spec = source.spectrum(
                    nu=source.emitting_molecule.nu0, output_type="Rayleigh-Jeans"
                )
                for Planck, RJ in zip((T_Planck, T_Planck_spec), (T_RJ, T_RJ_spec)):
                    intensity_from_T_Planck = helpers.B_nu(
                        T=Planck, nu=source.emitting_molecule.nu0
                    )
    ...
    def test_explicitly(self):
        ...
        for source in general_source_iterator(specie=self.specie, width_v=self.width_v):
            for T_dust, tau_dust in zip(
                (0, 45, lambda nu: np.ones_like(nu) * 87),
                ...
                S = tau_d * helpers.B_nu(
                    nu=nu, T=T_d
                ) + source.tau_nu0_individual_transitions * helpers.B_nu(
                    nu=nu, T=source.Tex
                )
                S /= tau_tot
                ...
                if source.geometry_name == "LVG sphere":
                    ...
                    specific_intensity = source.geometry.specific_intensity(
                        **intensity_kwargs
                    )
...
class TestPhysics:
    ...
    def cloud_iterator(self):
        for specie, width_v in zip(
            ("CO", "HCl"), (1 * constants.kilo, 20 * constants.kilo)
        ):
    ...
    def test_LTE_from_coll(self):
        collider_densities = {
            "para-H2": 1e11 / constants.centi**3,
        ...
        for source in self.cloud_iterator():
            ...
            assert np.allclose(
                source.level_pop, Boltzmann_level_population, atol=1e-5, rtol=1e-2
            )
    ...
    def test_LTE_from_ext_background(self):
        ...
        for source in self.cloud_iterator():
            ...
            source.solve_radiative_transfer()
            Boltzmann_level_population = (
                source.emitting_molecule.Boltzmann_level_population(T=Tbg)
            )
            assert np.allclose(
                source.level_pop, Boltzmann_level_population, atol=1e-5, rtol=1e-2
            )
...
class TestFrequencyIntegratedEmission:
    ...
    def test_wrong_input(self):
        ...
        with pytest.raises(ValueError):
            source.frequency_integrated_emission(output_type="flux")
        with pytest.raises(ValueError):
            source.frequency_integrated_emission(output_type="flux", solid_angle=None)
        with pytest.raises(ValueError):
            source.frequency_integrated_emission(output_type="asdf")

    def test_with_physics_thin(self):
        N = 1e12 / constants.centi**2
        for line_profile in ("Gaussian", "rectangular"):
            source = self.generate_and_solve_source(
                geometry="static sphere", N=N, line_profile_type=line_profile
            )
    ...
    def test_with_physics_thick(self):
        ...
        source = self.generate_and_solve_source(
            geometry="static slab", N=N, line_profile_type="rectangular"
        )
        ...
        for i, line in enumerate(source.emitting_molecule.rad_transitions):
            ...
            expected_intensity = (
                expected_specific_intensity * line.line_profile.width_nu
            )
