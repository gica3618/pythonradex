...
class Source:
    ...
    line_profiles = {
        "Gaussian": atomic_transition.GaussianLineProfile,
    ...
    def __init__(
        ...
        if verbose:
            for i in range(len(self.emitting_molecule.overlapping_lines)):
                if len(self.emitting_molecule.overlapping_lines[i]) > 0:
                    print(
                        f"identified overlaps for line {i}:"
        ...
        self.Einstein_kwargs = {
            "B12": self.emitting_molecule.B12,
    ...
    def check_parameters(self, collider_densities, T_dust, tau_dust, ext_background):
        if collider_densities is not None:
            ...
            for collider in collider_densities.keys():
                if collider not in available_colliders:
                    raise ValueError(
                        f'collider "{collider}" not available'
        ...
        if "LVG" in self.geometry_name:
            if not (T_dust in (None, 0) and tau_dust in (None, 0)):
                ...
                # if there is dust
                raise ValueError(
                    "including dust continuum is currently not"
                    + " supported for LVG geometries"
                )
    ...
    def update_parameters(
        ...
        if not hasattr(self, "rate_equations"):
            ...
            update_tau_dust = self.should_be_updated(
                new_func_or_number=tau_dust, old=self.rate_equations.tau_dust
            )
    ...
    def get_initial_level_pop(self):
        ...
        Boltzmann_level_population = self.emitting_molecule.Boltzmann_level_population(
            T=self.rate_equations.Tkin
        )
    ...
    def solve_radiative_transfer(self):
        ...
        while residual > self.relative_convergence or counter < self.min_iter:
            ...
            level_pop = (
                self.underrelaxation * new_level_pop
                + (1 - self.underrelaxation) * level_pop
            )
            if (
                self.use_Ng_acceleration
                and counter > self.min_iter_before_ng_acceleration
                and counter % self.ng_acceleration_interval == 0
            ):
                level_pop = self.ng_accelerate(
                    level_pop=level_pop, old_level_pops=old_level_pops
                )
        if self.verbose:
        ...
        if self.warn_negative_tau:
            if np.any(self.tau_nu0_individual_transitions < 0):
                negative_tau_transition_indices = np.where(
                    self.tau_nu0_individual_transitions < 0
                )[0]
                negative_tau_transitions = [
                    self.emitting_molecule.rad_transitions[i]
                    for i in negative_tau_transition_indices
                ]
                warnings.warn("negative optical depth!")
                for i, trans in zip(
                    negative_tau_transition_indices, negative_tau_transitions
                ):
                    print(
                        f"{trans.name}: tau_nu0 = {self.tau_nu0_individual_transitions[i]:.3g}"
                    )
        ...
        lower_level_population = [
            self.level_pop[t.low.index] for t in self.emitting_molecule.rad_transitions
        ]
        self.lower_level_population = np.array(lower_level_population)
        upper_level_population = [
            self.level_pop[t.up.index] for t in self.emitting_molecule.rad_transitions
        ]
    ...
    def get_solid_angle_warning_text(output_type):
        ...
        return (
            "solid_angle specified, but not needed for"
    ...
    def emission_at_line_center(self, output_type, transitions=None, solid_angle=None):
        ...
        if self.emitting_molecule.any_line_has_overlap(line_indices=transitions):
            ...
            specific_intensity_nu0 = (
                self.intensity_calculator.specific_intensity_nu0_no_overlap(
                    transitions=transitions
                )
            )
        if output_type == "specific intensity":
            ...
            out = self.transform_specific_intensity(
                specific_intensity=specific_intensity_nu0,
                nu=nu0s,
                solid_angle=solid_angle,
                output_type=output_type,
            )
    ...
    def print_results(self):
        ...
        print("\n")
        print(
            "  up   low      nu0 [GHz]    T_ex [K]      poplow         popup"
            + "         tau_nu0"
        )
        for i, line in enumerate(self.emitting_molecule.rad_transitions):
            output = (
                f"{line.up.index:>4d} {line.low.index:>4d} "
