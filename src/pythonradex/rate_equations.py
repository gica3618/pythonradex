...
class RateEquations:
    ...
    def set_ext_background(self, ext_background):
        ...
        if not self.treat_line_overlap:
            if isinstance(ext_background, numbers.Number):
                ...
                self.Iext_nu0 = np.full(
                    shape=self.molecule.nu0.shape, fill_value=ext_background
                )
            else:
                self.Iext_nu0 = np.array(
                    [self.ext_background(nu0) for nu0 in self.molecule.nu0]
                )
    ...
    def set_dust(self, T_dust, tau_dust):
        ...
        if not self.treat_line_overlap:
            if isinstance(tau_dust, numbers.Number):
                # fast:
                self.tau_dust_nu0 = np.full(
                    shape=self.molecule.nu0.shape, fill_value=tau_dust
                )
            else:
    ...
    def Ieff_nu0(
        ...
        for i in range(len(trans_low_index)):
            ...
            if surely_no_dust or tau_tot == 0:
                ...
                psistar_eta_c = (
                    (1 - beta_nu0[i]) * S_dust_nu0[i] * tau_dust_nu0[i] / tau_tot
                )
    ...
    def mixed_term_nu0(
        ...
        for i in range(len(trans_low_index)):
            ...
            if tau_tot == 0:
                ...
                term[i_up, i_low] = (
                    (1 - beta_nu0[i]) * tau_line_nu0[i] / tau_tot * A21[i]
                )
        return term
    ...
    def GammaR_nu0(self, level_population):
        ...
        Ieff_nu0 = self.Ieff_nu0(
            ...
            S_dust_nu0=self.S_dust_nu0,
            tau_dust_nu0=self.tau_dust_nu0,
    ...
    def get_tau_tot_functions(self, level_population):
        ...
        return [
            self.molecule.get_tau_tot_nu(line_index=i, **kwargs)
    ...
    def mixed_term_averaged(self, tau_tot_functions, tau_line_functions):
        ...
        for i, trans in enumerate(self.molecule.rad_transitions):
            ...
            pairing_transitions = self.molecule.overlapping_lines[i] + [
                i,
            ]
            for j in pairing_transitions:
                ...
                if trans.up.index != pairing_trans.low.index:
                    mixed_term[
                        trans.up.index, pairing_trans.low.index
                    ] += mixed_averaged
                if trans.up.index != pairing_trans.up.index:
                    mixed_term[
                        trans.up.index, pairing_trans.up.index
                    ] += -mixed_averaged
        return mixed_term
    ...
    def GammaR_averaged(self, level_population):
        ...
        mixed_term = self.mixed_term_averaged(
