...
class GaussianLineProfile(LineProfile):
    ...
    def average_over_phi_nu(self, func):
        ...
        while residual > 1e-2:
            ...
            nu = np.linspace(
                self.nu0 - width / 2 * self.width_nu,
                self.nu0 + width / 2 * self.width_nu,
                n_elements,
            )
            ...
            residual = helpers.relative_difference(
                a=np.array((new_average,)), b=np.array((old_average,))
            )
...
class Transition:
    ...
    def Tex(self, x1, x2):
        ...
        """
        return Tex(Delta_E=self.Delta_E, g_up=self.up.g, g_low=self.low.g, x1=x1, x2=x2)
...
class RadiativeTransition(Transition):
    ...
    def source_function(self, x1, x2):
        S = np.where(
            (x1 == 0) & (x2 == 0), 0, self.A21 * x2 / (x1 * self.B12 - x2 * self.B21)
        )
        # if np.any(S < 0):
...
class EmissionLine(RadiativeTransition):
    ...
    def __init__(self, up, low, A21, line_profile_type, width_v, nu0=None):
        ...
        self.line_profile = line_profiles[line_profile_type](
            nu0=self.nu0, width_v=width_v
        )
        self.tau_kwargs = {"A21": self.A21, "g_up": self.up.g, "g_low": self.low.g}

    @classmethod
    def from_radiative_transition(
        cls, radiative_transition, line_profile_type, width_v
    ):
