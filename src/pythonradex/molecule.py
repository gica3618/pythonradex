...
class Molecule:
    ...
    def __init__(self, datafilepath, partition_function=None):
        ...
        try:
            ...
            data = LAMDA_file.read(datafilepath=datafilepath, read_frequencies=True)
        except:
            data = LAMDA_file.read(datafilepath=datafilepath, read_frequencies=False)
        ...
        self.gup_rad_transitions = np.array(
            [line.up.g for line in self.rad_transitions]
        )
        self.glow_rad_transitions = np.array(
            [line.low.g for line in self.rad_transitions]
        )
        self.ilow_rad_transitions = np.array(
            [line.low.index for line in self.rad_transitions]
        )
        self.iup_rad_transitions = np.array(
            [line.up.index for line in self.rad_transitions]
        )
        self.Delta_E_rad_transitions = np.array(
            [line.Delta_E for line in self.rad_transitions]
        )
    ...
    def Z_from_atomic_data(self, T):
        ...
        if T.ndim > 0:
            shape = [
                ...
                1 for i in range(T.ndim)
            ]  # needs to come before T is modified
    ...
    def get_Tex(self, level_population):
        ...
        Tex = atomic_transition.Tex(
            Delta_E=self.Delta_E_rad_transitions,
...
class EmittingMolecule(Molecule):
    ...
    def __init__(
        ...
        """
        Molecule.__init__(
            self, datafilepath=datafilepath, partition_function=partition_function
        )
        ...
        self.phi_nu0 = np.array(
            [line.line_profile.phi_nu(line.nu0) for line in self.rad_transitions]
        )
        self.coll_gups = {}
        self.coll_glows = {}
        self.coll_DeltaEs = {}
        ...
        for collider, coll_transitions in self.coll_transitions.items():
            ...
            self.coll_DeltaEs[collider] = np.array(
                [c.Delta_E for c in coll_transitions]
            )
    ...
    def identify_overlapping_lines(self):
        ...
        for i, line in enumerate(self.rad_transitions):
            ...
            for j, overlap_line in enumerate(self.rad_transitions):
                if j == i:
                    continue
                no_overlap = (
                    overlap_line.line_profile.nu_min > line.line_profile.nu_max
        ...
        self.line_has_overlap = [
            len(self.overlapping_lines[i]) > 0 for i in range(self.n_rad_transitions)
        ]
    ...
    def set_K21_matrix(self):
        self.K21_matrix = {
            collider: np.zeros((len(coll_transitions), self.Tkin_data[collider].size))
            for collider, coll_transitions in self.coll_transitions.items()
        }
        for collider, coll_transitions in self.coll_transitions.items():
    ...
    def interpolate_K(self, Tkin, collider):
        ...
        K12 = atomic_transition.compute_K12(
            K21=K21,
            g_up=self.coll_gups[collider],
            g_low=self.coll_glows[collider],
            Delta_E=self.coll_DeltaEs[collider],
            Tkin=Tkin,
        )
        output["K21"] = K21
        output["K12"] = K12
    ...
    def get_GammaC(self, Tkin, collider_densities):
        ...
        for collider, coll_dens in collider_densities.items():
            ...
            K = self.construct_K_matrix(
                ...
                K12=interpK["K12"],
                K21=interpK["K21"],
                ilow=self.coll_ilow[collider],
                iup=self.coll_iup[collider],
            )
            GammaC += K * coll_dens
        return GammaC
