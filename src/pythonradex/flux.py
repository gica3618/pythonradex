...
def construct_rectangular_nu_and_tau(nu0,width_nu,tau_nu0,n_nu_elements):
    assert n_nu_elements%2 == 1,'need odd number of elements to include nu0 in the array'
    middle_index = n_nu_elements//2
    nu = np.linspace(nu0-width_nu/2,nu0+width_nu/2,n_nu_elements)
    tau_nu = 1/nu**2
    tau_nu *= tau_nu0/tau_nu[middle_index]
    return nu,tau_nu

@nb.jit(nopython=True,cache=True)
def get_flux_parameters_for_rectangular_flux(width_v,nu0,tau_nu0,Tex,n_nu_elements):
    width_nu = width_v/constants.c*nu0
    nu,tau_nu = construct_rectangular_nu_and_tau(
                   nu0=nu0,width_nu=width_nu,tau_nu0=tau_nu0,
                   n_nu_elements=n_nu_elements)
    source_function = helpers.B_nu(T=Tex,nu=nu)
...
class FluxCalculator():
    ...
    @staticmethod
    @nb.jit(nopython=True,cache=True)
    def fast_line_fluxes_rectangular_regular_without_overlap(
    ...
    @staticmethod
    @nb.jit(nopython=True,cache=True)
    def fast_line_fluxes_Gaussian_without_overlap(
                   solid_angle,transitions,tau_nu0_individual_transitions,Tex,nu0,
                   compute_flux_nu,width_v,tau_peak_fraction,nu_per_FHWM):
        obs_line_fluxes = []
        for i in transitions:
            tau_nu0 = tau_nu0_individual_transitions[i]
            FWHM_nu = width_v/constants.c*nu0[i]
            ...
            phi_nu_shape = np.exp(-(nu-nu0[i])**2/(2*sigma_nu**2))
    ...
    def fluxes_of_individual_transitions(self,solid_angle,transitions=None):
        ...
        if self.emitting_molecule.line_profile_type == 'Gaussian':
            fast_flux = self.fast_line_fluxes_Gaussian_without_overlap(
                           ...
                           Tex=self.Tex,width_v=self.emitting_molecule.width_v,
                           compute_flux_nu=self.compute_flux_nu,
