...
column_densities = {
    "treat overlap False": 2.11e13 / constants.centi**2 / constants.kilo * Doppler,
    "treat overlap True": 2.11e13 / constants.centi**2 / constants.kilo * Doppler,
}
molpop_cep_Tex = {
    "treat overlap False": [3.74e00, 3.70e00, 3.87e00, 4.28e00, 3.49e00, 3.60e00],
    "treat overlap True": [4.44e00, 4.57e00, 4.65e00, 4.71e00, 3.86e00, 4.01e00],
}
molpop_cep_tau_nu0 = {
    "treat overlap False": [3.53e-01, 4.10e-01, 9.71e-01, 1.90e00, 2.39e-02, 3.30e-01],
    "treat overlap True": [3.30e-01, 3.09e-01, 8.97e-01, 1.81e00, 2.30e-02, 3.14e-01],
}
...
molpop_cep_tau = {
    "treat overlap True": np.zeros_like(nu),
    "treat overlap False": np.zeros_like(nu),
}
molpop_cep_Stot = {
    "treat overlap True": np.zeros_like(nu),
    "treat overlap False": np.zeros_like(nu),
...
for treat_line_overlap in (True, False):
    ...
    for counter, i in enumerate(ref_transitions):
        ...
        print(
            f"tau_nu0={source.tau_nu0_individual_transitions[i]:.3g} (molpop: {tau_molpop:.3g})"
        )
