...
for treat_overlap in treat_line_overlap_values:
    ...
    for d, (dust_case, dust_p) in enumerate(dust_params.items()):
        ...
        if d == 0:
            ax.plot(
                src.emitting_molecule.Boltzmann_level_population(Tkin)[:max_plot_level],
                label="LTE Tkin",
                linestyle="dashed",
            )
            ax.plot(
                src.emitting_molecule.Boltzmann_level_population(T_dust_value)[
                    :max_plot_level
                ],
                label="LTE Tdust",
                linestyle="dashed",
