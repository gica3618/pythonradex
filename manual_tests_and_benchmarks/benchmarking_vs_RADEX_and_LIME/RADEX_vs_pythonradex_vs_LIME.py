...
filename = "co.dat"
Tkin = 100
coll_partner_density_cases = {
    "LTE": {"ortho-H2": 1e8 / constants.centi**3},
    "non-LTE": {"ortho-H2": 1e2 / constants.centi**3},
}
# coll_partner_density_cases = {'non-LTE':{'ortho-H2':1e2/constants.centi**3}}
N_cases = {
    "thick": 5e18 / constants.centi**2,
    "thin": 1e15 / constants.centi**2,
...
radex_wrapper_geo = {"sphere": "static sphere", "slab": "LVG slab"}
...
for N_case, LTE_case in itertools.product(N_cases, coll_partner_density_cases):
    ...
    for geo, line_profile_type in itertools.product(geometries, line_profile_types):
        ...
        pythonradex_nu0 = source.emitting_molecule.rad_transitions[
            trans_number
        ].line_profile.nu0
        lineprofile_nu0[line_profile_type] = source.emitting_molecule.rad_transitions[
            trans_number
    ...
    colliders = [
        pyLime.Collider(name="orthoH2", density=n_orthoH2),
    ]
    radiating_species = [
        pyLime.RadiatingSpecie(moldatfile=filepath, density=n),
    ]
    lime = pyLime.Lime(
        axes=axes,
        T=T,
        ...
        images=images,
        level_population_filename="levelpop.fits",
    ...
    lime_flux_density = np.trapz(
        np.trapz(output_flux.data, output_flux.z, axis=1), output_flux.x, axis=0
    )
    ...
    lime_Tex = np.where(
        low_pop > 0,
        -Delta_E / constants.k / np.log(low.g * up_pop / (up.g * low_pop)),
        0,
    )
    ...
    if N_case == "thick":
        ...
        up_level_pop = source.emitting_molecule.Boltzmann_level_population(Tkin)[
            up.index
        ]
        if general_geometry == "slab":
            thin_LTE_flux = (
    ...
    def split_thick_thin(obs, obs_name):
        params["{:s} rectangular".format(obs_name)] = {
            ...
            if "rectangular" in key or "black body" in key or key == "RADEX"
        }
