...
class GridFile:
    ...
    def write_grid_axes(self):
        for ax_name, ax in self.axes.items():
            ...
            self.add_constant(
                name="grid_{:s}_size".format(ax_name), datatype="int", value=ax.size
            )
    ...
    def write_grid_values(self, name, array, floor):
        ...
        self.add_constant(
            name="grid_{:s}_floor".format(name), datatype="double", value=floor
        )
...
class LimeImageParam:
    ...
    def input_str(self, img_number):
        return "img[{:d}].{:s} = {:s};\n".format(
            img_number, self.name, self.value_str()
        )
...
class LimeImage:
    ...
    def __init__(
        ...
        self.params = [
            IntegerLimeImageParam(name="nchan", value=nchan),
            ...
            FloatLimeImageParam(
                name="imgres", value=imgres, unit_change_factor=1 / constants.arcsec
            ),
...
class Lime:
    ...
    def grid_interpolation_string(name):
        return "grid_interpolation_3D(x, y, z, grid_{:s}, grid_{:s}_floor)".format(
            name, name
        )
    ...
    def write_grid_file(self):
        ...
        for collider in self.colliders:
            grid_file.write_grid_values(
                name=self.density_grid_str(collider.name),
                array=collider.density,
                floor=self.density_floor,
            )
        grid_file.write_grid_values(
            name="temperature", array=self.T, floor=self.T_floor
        )
        for radiating_specie in self.radiating_species:
            grid_file.write_grid_values(
                name=self.nmol_grid_str(radiating_specie.moldatfile),
                array=radiating_specie.density,
                floor=self.nmol_floor,
            )
        for ax in ("x", "y"):
            grid_file.write_grid_values(
                name="velocity_{:s}".format(ax),
                array=self.velocity[ax],
    ...
    def write_LIME_input_file(self):
        ...
        for line in template:
            if "par->radius" in line:
                input_file.write(
                    "par->radius = {:g}*AU;\n".format(self.radius / constants.au)
                )
...
class LimeFitsOutput:
    ...
    def determine_axes(self):
        ...
        for i in range(1, 4):
            pix_ax = (
                np.arange(self.header["NAXIS{:d}".format(i)])
    ...
    def mom0like_plot(self, data, title=None):
        fig, ax = plt.subplots()
        plot = ax.pcolormesh(
            self.X / constants.arcsec, self.Z / constants.arcsec, data, shading="auto"
        )
        ax.set_title(title)
...
class LimeFitsOutputFlux(LimeFitsOutput):
    ...
    def plot_pv(self, title=None):
        plt.figure()
        plot = plt.pcolormesh(
            self.P / constants.arcsec,
            self.V / constants.kilo,
            self.pv_diagram,
            shading="auto",
        )
        plt.title(title)
...
class LimeFitsOutputFluxSI(LimeFitsOutputFlux):
    ...
    def total_flux(self):
        return np.trapz(
            np.trapz(-np.trapz(self.data, self.nu, axis=-1), self.z, axis=-1), self.x
        )
...
if __name__ == "__main__":
    ...
    ne += (
        30
        * np.exp(-((x3D - x0) ** 2) / (2 * sigma_x**2))
        * np.exp(-((y3D - y0) ** 2) / (2 * sigma_x**2))
    )
    ...
    radiating_species = [
        RadiatingSpecie(moldatfile=os.path.join(datafolder, "c.dat"), density=ne),
        RadiatingSpecie(moldatfile=os.path.join(datafolder, "c+.dat"), density=ne),
    ]
    Mstar = 2e30 * 1.75
    ...
    general_img_kwargs = {
        ...
        "trans": 0,
        "pxls": 200,
        "imgres": 0.1 * constants.arcsec,
        "distance": 20 * constants.parsec,
        "phi": 0,
        "units": "2 4",
    }
    image_c = LimeImage(theta=0, filename="test_c.fits", molI=0, **general_img_kwargs)
    image_cplus = LimeImage(
        theta=0, filename="test_c+.fits", molI=1, **general_img_kwargs
    )
