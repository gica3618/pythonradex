...
def run_models(width_v, collider_densities, N, run_only_with_overlap_treatment=False):
    for treat_line_overlap in (False, True):
        ...
        source = radiative_transfer.Source(
            **cloud_kwargs, width_v=width_v, treat_line_overlap=treat_line_overlap
        )
        source.update_parameters(
            **param_kwargs, collider_densities=collider_densities, N=N
        )
        ...
        for i in ref_transitions:
            ...
            print(
                f"up,low: {source.level_pop[trans.up.index]:.3g}, {source.level_pop[trans.low.index]:.3g}"
            )
        print("\n")
    print("\n\n")
...
run_models(
    ...
    collider_densities={
        collider: 1e5 / constants.centi**3 for collider in ("ortho-H2", "para-H2")
    },
...
run_models(
    width_v=width_v_partial_overlap,
    collider_densities=coll_dens_nonLTE,
    N=N_partial_overlap,
)
...
coll_dens_LTE = {
    collider: 1e9 / constants.centi**3 for collider in ("ortho-H2", "para-H2")
}
run_models(
    width_v=width_v_partial_overlap,
    collider_densities=coll_dens_LTE,
    N=N_partial_overlap,
)

print("Case 5: total overlap LTE")
run_models(
    width_v=width_v_total_overlap, collider_densities=coll_dens_LTE, N=N_total_overlap
)
...
run_models(
    ...
    run_only_with_overlap_treatment=True,
)
