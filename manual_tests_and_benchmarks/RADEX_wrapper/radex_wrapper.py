...
class RadexInput:
    ...
    def write_input_file(self):
        ...
        input_file.write(
            "{:f} {:f}\n".format(
                self.frequency_interval.min / constants.giga,
        ...
        input_file.write(
            "{:f}\n".format(self.Delta_v / constants.kilo)
        )  # line width in km/s
...
class RadexOutput:
    ...
    def read(self):
        ...
        if output_lines[-2].split()[-1] != "(erg/cm2/s)":
            error_message = (
                "Error: Ambiguous line selection (no line or more than one line)\n"
                + "See {:s} to find out".format(self.output_filepath)
            )
            raise RuntimeError(error_message)
        results = output_lines[-1].split()
        output = {
            "flux": float(results[-1]) * constants.erg / constants.centi**2,
            "tau": float(results[-6]),
            "Tex": float(results[-7]),
            "TR": float(results[-5]),
            "pop_up": float(results[-4]),
...
class RadexWrapper:
    ...
    def __init__(self, geometry):
        ...
        executables = {
            "static sphere": "radex_static_sphere",
            "LVG sphere": "radex_LVG_sphere",
            "LVG slab": "radex_LVG_slab",
        }
        exec_paths = {
            ID: os.path.join(folderpath, f"../../tests/Radex/bin/{ex}")
            for ID, ex in executables.items()
        }
...
if __name__ == "__main__":
    ...
    test_radex_input = RadexInput(
        data_filename="hco+.dat",
        frequency_interval=test_freq_interval,
        Tkin=30,
        coll_partner_densities=test_coll_partners,
        T_background=2.73,
        column_density=1e14 * constants.centi**-2,
