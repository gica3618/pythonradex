...
def flux_static_sphere(tau_nu0):
    flux_nu0 = 2*np.pi*source_function_nu0/tau_nu0**2\
               *(tau_nu0**2/2-1+(tau_nu0+1)*np.exp(-tau_nu0)) *solid_angle/np.pi
    #assume rectangular profile:
    return flux_nu0*width_nu

def flux_lvg_sphere(tau_nu0):
    #see LVG_sphere.pdf
    return nu0/constants.c*solid_angle*source_function_nu0*(1-np.exp(-tau_nu0))\
           *4/3*V
...
pythonradex_fluxes = {"static sphere":flux_static_sphere(tau_nu0=tau_values),
