...
beta_funcs = {
    "static sphere": escape_probability_functions.beta_static_sphere,
...
for i, n in enumerate(n_values):
    ...
    for ID, beta_func in beta_funcs.items():
        ...
        pythonradex_fluxes[ID][i] = (
            np.trapezoid(specific_intensity_pythonradex, nu) * solid_angle
        )  # W/m2
...
for ID in beta_funcs.keys():
    ...
    ax.plot(
        n_values / constants.centi**-3,
        beta_fluxes[ID],
        label="flux from beta",
        linestyle="dashed",
    )
