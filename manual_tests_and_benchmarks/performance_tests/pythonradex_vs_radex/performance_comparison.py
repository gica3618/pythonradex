...
for grid_name, grid in grids.items():
    ...
    for geometry in geometries:
        ...
        for line_profile_type in ("Gaussian", "rectangular"):
            ...
            for coll_dens, Tkin, N in itertools.product(
                coll_density_values, Tkin_values, N_values
            ):
                collider_densities = {
                    collider: coll_dens for collider in grid["colliders"]
                }
    ...
    for geometry in geometries:
        ...
        for N, coll_dens, Tkin in itertools.product(
            N_values, coll_density_values, Tkin_values
        ):
            ...
            wrapper_times = mini_radex_wrapper.run_radex(
                datafilename=grid["data_filename"],
                geometry=geometry,
                collider_densities=collider_densities,
                Tkin=Tkin,
                N=N,
                width_v=width_v,
                input_filepath=radex_input_file,
...
for grid_name in grids.keys():
    print(grid_name)
    time_collections = {
        "pythonradex": pythonradex_times[grid_name],
        "RADEX": radex_times[grid_name],
    }
    average_times[grid_name] = {
        code: np.mean(list(tc.values())) for code, tc in time_collections.items()
    }
